    ...
    for (i = 0; i < img.rows; i++) {
        for (j = 0; j < img.cols; j += XF_NPPC) {
            ...
            for (l = 0; l < XF_NPPC; l++) {
                ...
                switch (depth) {
                    case 10:
                        xf::cv::AXISetBitFields(axi, (l)*depth, depth, cv_pix);
                        break;
                    case 12:
                        xf::cv::AXISetBitFields(axi, (l)*depth, depth, cv_pix);
                        break;
...
int main(int argc, char** argv) {
    ...
    std::fread(dims, sizeof(*dims), sizeof(dims), file);
    const int delta = 2132 - 2064;
    dims[1] -= delta;
    raw_input.create(dims[1], dims[0], CV_16UC1);
    auto ptr = raw_input.ptr();
    // skip delta
    std::fread(ptr, sizeof(short), dims[0] * delta, file);
    std::fread(ptr, sizeof(short), dims[0] * dims[1], file);
    std::fclose(file);
...
#else
    final_output.create(raw_input.rows, raw_input.cols, CV_16UC3);
#endif
    ...
    for (int i = 0; i < 32; i++) {
        if ((1 << i) > (imgSize.height / 2)) break;
        blk_height = (1 << i);
    }

    for (int i = 0; i < 32; i++) {
        if ((1 << i) > (imgSize.width / 2)) break;
        blk_width = (1 << i);
    }

    std::cout << blk_height <<", " << blk_width <<std::endl;

    float gamma_val_r = 2.2f, gamma_val_g = 2.2f, gamma_val_b = 2.2f;
    ...
    mode_reg = 0;
        ...
        ISPPipeline_accel(raw_input.cols, raw_input.rows, src_axi, dst_axi, rgain, bgain, blk_height, blk_width, gamma_lut, mode_reg, pawb);
    ...
    imwrite("output_plain.png", final_output);
    ...
    imwrite("output_gamma.png", final_output);
    ...
    imwrite("output_awb.png", final_output);
    ...
    imwrite("output_gamma_awb.png", final_output);
