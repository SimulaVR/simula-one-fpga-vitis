...
template <int SRC_T, int DST_T, int ROWS, int COLS, int NPC = 1, int XFCVDEPTH_DEMOSAIC_OUT, int XFCVDEPTH_LTM_IN>
void fifo_downsample(xf::cv::Mat<XF_10UC3, ROWS, COLS, NPC, XFCVDEPTH_DEMOSAIC_OUT>& demosaic_out,
               xf::cv::Mat<XF_8UC3, ROWS, COLS, NPC, XFCVDEPTH_LTM_IN>& ltm_in,
               unsigned short height,
               unsigned short width) {
// clang-format off
#pragma HLS INLINE OFF
	int IN_STEP = XF_DTPIXELDEPTH(XF_10UC3, NPC);
	int OUT_STEP = XF_DTPIXELDEPTH(XF_8UC3, NPC);
    // clang-format on
    ap_uint<13> row, col;
    ap_uint<6> i;
    int readindex = 0, writeindex = 0;
...
#pragma HLS pipeline
            // clang-format on
            XF_TNAME(XF_10UC3, NPC) tmp_src;
            tmp_src = demosaic_out.read(readindex++);
            XF_TNAME(XF_8UC3, NPC) tmp_dst;
            for (i = 0; i < XF_CHANNELS(XF_10UC3, NPC) * XF_NPIXPERCYCLE(NPC); i++) {
            		auto val = ap_uint<10>(tmp_src.range(IN_STEP*(i+1)-1, IN_STEP*i));
            		tmp_dst.range(OUT_STEP*(i+1) - 1, OUT_STEP*i) = val >> 2;// hls::min(val, ap_uint<10>(255)); // val >> 2
            }
            ltm_in.write(writeindex++, tmp_dst);
        }
    }
}
...
template <int SRC_T, int DST_T, int ROWS, int COLS, int NPC = 1, int XFCVDEPTH_DEMOSAIC_OUT, int XFCVDEPTH_LTM_IN>
void function_gamma(xf::cv::Mat<SRC_T, ROWS, COLS, NPC, XFCVDEPTH_DEMOSAIC_OUT>& aecin,
                  xf::cv::Mat<DST_T, ROWS, COLS, NPC, XFCVDEPTH_LTM_IN>& _dst,
...
#pragma HLS INLINE OFF
    ...
    ap_uint<8> mode = (ap_uint<8>)mode_reg;
    ap_uint<1> mode_flg = mode.range(1, 1);
    ...
    if (mode_flg) {
        ...
        fifo_copy<XF_LTM_T, XF_LTM_T, XF_HEIGHT, XF_WIDTH, XF_NPPC, XF_CV_DEPTH_AEC_IN, XF_CV_DEPTH_DST>(
            aecin, _dst, height, width);
    }
}
...
#pragma HLS INLINE OFF
    ...
    xf::cv::Mat<XF_DST_T, XF_HEIGHT, XF_WIDTH, XF_NPPC, XF_CV_DEPTH_IMPOP> filter_out(height, width);
...
//                                  XF_CV_DEPTH_LSC_OUT>(ltm_in, lsc_out);
    ...
    fifo_downsample<XF_DST_T, XF_LTM_T, XF_HEIGHT, XF_WIDTH, XF_NPPC, XF_CV_DEPTH_LSC_OUT, XF_CV_DEPTH_AEC_IN>(
    		ltm_in, aecin, height, width);

    //xf::cv::xf_QuatizationDithering<XF_DST_T, XF_LTM_T, XF_HEIGHT, XF_WIDTH, 256, Q_VAL, XF_NPPC, XF_CV_DEPTH_LSC_OUT, XF_CV_DEPTH_AEC_IN>(
    //		ltm_in, aecin);

    //xf::cv::LTM<XF_DST_T, XF_LTM_T, BLOCK_HEIGHT, BLOCK_WIDTH, XF_HEIGHT, XF_WIDTH, XF_NPPC, XF_CV_DEPTH_LTM_IN, XF_CV_DEPTH_AEC_IN>::process(
    //		ltm_in, blk_height, blk_width, omin_r, omax_r, omin_w, omax_w, aecin);



    function_gamma<XF_LTM_T, XF_LTM_T, XF_HEIGHT, XF_WIDTH, XF_NPPC, XF_CV_DEPTH_AEC_IN, XF_CV_DEPTH_DST>(
    		aecin, _dst, gamma_lut, height, width, mode_reg);
...
#pragma HLS ARRAY_PARTITION variable=omax dim=3   cyclic factor=2
    ...
    if (!flag) {
        ISPpipeline(s_axis_video, m_axis_video, height, width, hist0_awb, hist1_awb, igain_0, igain_1, rgain, bgain,
        			omin[0], omax[0], omin[1], omax[1], blk_height, blk_width,
        ...
        ISPpipeline(s_axis_video, m_axis_video, height, width, hist1_awb, hist0_awb, igain_1, igain_0, rgain, bgain,
        			omin[1], omax[1], omin[0], omax[0], blk_height, blk_width,
                    gamma_lut, mode_reg, pawb);
        flag = 0;
    }
}
